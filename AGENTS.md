...
- Configs: `tsconfig.json`, `eslint.config.mjs`, `.prettierrc.yml`, `.commitlintrc.yaml`, `lefthook.yml`.
...
- Lint/format locally when needed: `npx eslint --fix .` and `npx prettier --write .` (lefthook runs these automatically on staged files if installed).
...
- ESLint: prefer type-only imports, forbid `console.*` (use a logger or pass one in), disallow unused vars unless prefixed with `_`.
